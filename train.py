    ...
    for epoch in range(last_epoch+1, epochs):
        ...
        with tqdm(dataloader, desc=f"Epoch {epoch+1}/{epochs} - Train") as pbar:
            for idx, batch in enumerate(pbar, 1):
                ...
                mel_l1 = F.smooth_l1_loss(y_g_hat, y, beta=0.25) #F.l1_loss(y, y_g_hat) + F.mse_loss(y, y_g_hat) * 0.2 #F.smooth_l1_loss(y, y_g_hat)
                epoch_loss_only += mel_l1.item()
                spec_loss = g_spec_loss(y, y_g_hat)
                loss_total = mel_l1 + spec_loss * 0.003
                optim_g.zero_grad()
                loss_total.backward()
                optim_g.step()
...
if __name__ == "__main__":
    ...
    train_vocoder(h, dataloader, "./checkpoints_finetune", epochs=201, checkpoint_interval=300, new_learning_rate=0.00005)